...
def frameAP(groundtruth, detections, thr, print_info=True):
    ...
    results = {}
    train_label_file = open("/work/sc19981018/webber/ucf_label_split/75vs25/2/train_label.txt", "r")
    test_label_file = open("/work/sc19981018/webber/ucf_label_split/75vs25/2/test_label.txt", "r")
    ...
    for ilabel, label in enumerate(GT['labels']):
        ...
        if label not in our_test_labels:
            if print_info:
                print('do not evaluate{}'.format(label))
            continue
        detections = alldets[alldets[:, 2] == ilabel, :]

        this_class_detections = alldets[alldets[:, -1] == ilabel, :]
        correct = [item for item in this_class_detections if (item[-1] == item[2])]
        accuracy.append(len(correct) / len(this_class_detections))
    ...
    if print_info:
        ...
        print("{:20s} {:8.2f}".format("mac", accuracy))
