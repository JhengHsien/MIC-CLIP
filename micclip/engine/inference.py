...
from micclip.dataset.datasets.evaluation import evaluate
from micclip.utils.comm import get_rank, is_main_process, all_gather, gather, synchronize, get_world_size
...
def compute_on_dataset_2stage(model, data_loader, device, logger):
    ...
    for i, batch in enumerate(tqdm(data_loader, **extra_args)):
        boxes, objects, extras, video_ids = batch
        # print(extras)
        # slow_clips = slow_clips.to(device)
        # fast_clips = fast_clips.to(device)
        boxes = [box.to(device) for box in boxes]
        ...
        with torch.no_grad():
            feature = model(boxes, objects, part_forward=0)
        ...
        batch_info_list[i] = (movie_ids, timestamps, video_ids, object_feature, [b.extra_fields['det_score'] for b in boxes])
    ...
    start_time = time.time()
    for movie_ids, timestamps, video_ids, object_feature, det_scores in tqdm(batch_info_list, **extra_args):
        ...
        with torch.no_grad():
            output = model(None, None, extras=extras, part_forward=1)
            cache = get_local.cache
            
            import json
            with open('data.json', 'w', encoding='utf-8') as f:
                json.dump(cache, f, default=json_serialize)
