...
from micclip.utils.metric_logger import MetricLogger
from micclip.engine.inference import inference
from micclip.utils.comm import synchronize, reduce_dict, all_gather
...
def do_train(
    ...
    for iteration, (boxes, objects, extras, _) in enumerate(data_loader, start_iter):
        ...
        loss_dict, weight_dict, metric_dict, pooled_feature = model(boxes, objects, mem_extras)
        ...
        for keys in loss_dict:
            if (keys not in history.keys()):
                history[keys] = []
            history[keys].append(loss_dict[keys] * weight_dict[keys])
