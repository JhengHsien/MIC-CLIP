...
from micclip.utils.IA_helper import has_memory, has_person, has_object, has_hand
import time
...
class InteractionUnit(nn.Module):
    ...
    def forward(self, person, others):
        ...
        # print(max_others, self.max_others)
        ...
        middle_ = torch.bmm(weight, value_batch)  # n, thw, dim_inner

        out = middle_.contiguous().view(n, t * h * w, -1)
...
class HITStructure(nn.Module):
    def __init__(self, dim_person, dim_mem, dim_out, structure_cfg):
        ...
        self.max_scene = 1
        self.mem_len = structure_cfg.LENGTH[0] + structure_cfg.LENGTH[1] # 8
        self.mem_feature_len = self.mem_len  * structure_cfg.VIDEOS_PER_BATCH
        ...
        # Init Scene
        self.scene_dim_reduce = nn.Conv3d(dim_person, self.dim_inner, 1, bias)
        init_layer(self.scene_dim_reduce, conv_init_std, bias)
            
                        
    def forward(self, person, person_boxes, obj_feature, object_boxes, scene_feature, mem_feature, person_pooled, phase):
        # RGB stream
        if phase == "rgb":
            query, person_key, object_key, scene_key, mem_key = self._reduce_dim(person, person_boxes, obj_feature, object_boxes, scene_feature,
                                                                    mem_feature, phase)

            return self._aggregate(person_boxes, query, person_key, object_key, scene_key, mem_key)
        else:
            # change to 1024
            if (person_pooled.shape[1] == 2304):
                person_pooled = self.conv_reduce(person_pooled)
            if (obj_feature.shape[1] == 2304):
                obj_feature = self.conv_reduce(obj_feature)
            if (scene_feature.shape[1] == 2304):
                scene_feature = self.conv_reduce(scene_feature) 
                
            person_key = person_pooled
            
            query, _, object_key, scene_key, mem_key = self._reduce_dim(person, person_boxes, obj_feature, object_boxes, scene_feature,
                                                                    mem_feature, phase)
            pose_feats, _, _, _ = self._aggregate(person_boxes, query, person_key, object_key, scene_key, mem_key)
            
            # Attentive Fusion and Temporal Interaction
            return self._fuse(pose_feats, pose_feats, mem_key)
            
    

    def _reduce_dim(self, person, person_boxes, obj_feature, object_boxes, scene_feature, mem_feature, phase):
        ...
        if self.has_M and mem_feature != None:
            ...
            mem_key = unfuse_batch_num(mem_key, n, mem_key.size(0)/n)
        else:
            mem_key = None

        return query, person_key, object_key, scene_feature, mem_key

    def _aggregate(self, proposals, query, person_key, object_key, scene_key, mem_key):
...
class SerialHITStructure(HITStructure):
    ...
    def _aggregate(self, person_boxes, query, person_key, object_key, scene_key ,mem_key):
        block_count = dict()
        concat_query = query
        for idx, block_type in enumerate(self.I_block_list):
            block_count[block_type] = block_count.get(block_type, 0) + 1
            ...
            if block_type == "O":
                ...
                query = I_block(query, scene_key)
            
            elif block_type == "M":
                query = I_block(query, mem_key)
            person_key = query

        #     concat_query = torch.cat((concat_query, query), dim=1)
        # concat_query = concat_query.reshape(concat_query.shape[0], -1)
        # concat_query = concat_query.permute(2,3,4,1,0).contiguous().cuda()
        
        # concat_query = self.lin(concat_query)
        # concat_query = concat_query.unsqueeze(2).unsqueeze(2).unsqueeze(2)
...
class HITNet(SerialHITStructure):
    ...
    def __init__(self, dim_person, dim_mem, dim_out, structure_cfg):
        ...
        init_tensor_lateral = torch.full((4, 1024, 1, 1, 1), -5 * math.log(10), dtype=torch.float32)
        self.dense_params_lateral = nn.Parameter(init_tensor_lateral)
        
        self.softmax = nn.Softmax(dim=0)
        self.conv_reduce = nn.Conv3d(dim_mem, 1024, 1, bias=False)
        self.conv_reduce_fuse = nn.Conv3d(2048, 1024, 1, bias=False)
        

        self.non_local = NL(hidden_dim = 2048, out_dim = 1024)

    # Lateral units. They will serve as inputs to the intra-modality aggregator
    def aggregate_lateral(self, I_block_object, I_block_scene, I_block_memory, query, object_key, scene_key, mem_key):
        object_feat= I_block_object(query, object_key)
        scene_feat = I_block_scene(query, scene_key)
        mem_feat = I_block_memory(query, mem_key)
        return object_feat, mem_feat, keypoint_feat
    ...
    def _aggregate(self, person_boxes, query, person_key, object_key, scene_key, mem_key):
        ...
        for idx, block_type in enumerate(self.I_block_list):
            ...
            # Initialize the blocks. K stands for hands
            ...
            name_scene = 'S' + "_block_{}".format(block_count[block_type])
            I_block_scene = getattr(self, name_scene)
            ...
            elif block_type == "O": # receive P --> (O, K, M)
                P_O, P_M, P_K = self.aggregate_lateral(I_block_object, I_block_scene, I_block_memory, query, object_key, keypoints_key, mem_key)
                if P_M.shape[1] == 2304:
        ...
        return query, person_message, object_message, keypoints_message
...
def unfuse_batch_num(things, batch_size, num):
    ...
    _, c, t, h, w = things.size()
    return things.view(batch_size, int(num), c, t, h, w)
